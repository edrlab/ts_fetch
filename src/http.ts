...
import {
    IHttpGetResult, IAuthenticationToken, THttpGetCallback, THttpOptions, THttpResponse,
} from "./http.type";
...
type THttpFetchFormattedResponse = <TData = undefined>(url: string | URL, options?: RequestInit | undefined, callback?: THttpGetCallback<TData> | undefined, locale?: string | undefined) => Promise<IHttpGetResult<TData>>;
...
export class http implements IHttp {
    ...
    private async httpFetchRawResponse(
        ...
        if (isRedirect(response.status)) {
            ...
            if (location) {
                ...
                return await this.httpFetchRawResponse(locationUrl, options, redirectCounter + 1, locale);
            } else {
    ...
    private httpGetWithAuth = (enableAuth = true): THttpFetchFormattedResponse =>
        async (...arg) => {
            ...
            if (enableAuth) {
                ...
                if (
                    ...
                    return this.httpGetUnauthorized(auth)(_url, options, _callback, ..._arg);
                }
            ...
            // return response;
            return this.httpFetchFormattedResponse(
                _url,
                options,
                _callback,
                ..._arg,
    ...
    (auth: IAuthenticationToken, enableRefresh = true): THttpFetchFormattedResponse =>
        async (...arg) => {
            ...
            if (enableRefresh) {
                if (response.statusCode === 401) {
                    if (auth.refreshUrl && auth.refreshToken) {
                        try {
                            const responseAfterRefresh = await this.httpGetUnauthorizedRefresh(
                                auth,
                            )(url, options, _callback, _auth, ..._arg);
                        ...
                        try {
                            ...
                            const responseWithoutAuth = await this.httpGetWithAuth(
                                false,
                            )(url, options, _callback, undefined, ..._arg);
    ...
    private httpGetUnauthorizedRefresh =
        (auth: IAuthenticationToken): THttpFetchFormattedResponse =>
            ...
            if (httpPostResponse.isSuccess && httpPostResponse.response) {
                ...
                const httpGetResponse = await this.httpGetUnauthorized(auth, false)(...arg);
    ...
    public post: THttpFetchFormattedResponse =
        async (...arg) => {
